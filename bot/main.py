...
def getDisciplinesArray(disciplinesElements, title):
    parsedText = disciplinesElements \
    .replace(';', '') \
    .replace('\n', '') \
    .replace(' :', ':') \
    .replace(': ', ':') \
    ...
    .replace('Sala', ';Sala') \
    .split('#DIV##')
    dayData = [x[1:].split(';') for x in parsedText[1:]]
    disciplines = dict()
...
def getFullTimetable(bot, update):
    ...
    for weekday in days_of_week:
        ...
        weekday.h2.extract()
        dayData = getDisciplinesArray(weekday.text, title)        
